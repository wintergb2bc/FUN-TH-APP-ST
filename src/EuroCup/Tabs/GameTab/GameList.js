...
// import { checkIsLogin, redirectToLogin } from "../../../containers/SbSports/lib/js/util";
...
class GameList extends React.Component {
	constructor(props) {
		...
		this.weekDayMap = { 1: 'T. Hai', 2: 'T. Ba', 3: 'T. Tư', 4: 'T. Năm', 5: 'T. Sáu', 6: 'T. Bảy', 7: 'CN' }; //用moment的isoWeekday去對  1 being Monday and 7 being Sunday.
	...
	/* 存储单投和串关数据 */
	PushBetCart(data, type, lengths = 0) {
		...
		if (indexOf != '-1') {
			...
			if(lengths + 1 > this.props.Vendor.configs.MaxParlay) {
				Toasts.error(' Số lượng cược tối đa có thể đóng' + this.props.Vendor.configs.MaxParlay);
				return
			}
			this.setState(
				{
		...
		type == 2 && Toasts.success('Thêm thành công');
	};
	...
	clickOdds(selectionData) {
		...
		// 	return;
		// }
		PiwikMenberCode('Engagement Event','Click','Odds_EUROPage')
		...
		if (!MoreStatus) {
			...
			if (indexOf === -1) {
				this.PushBetCart(selectionData, 2, BetCartdata.length,);

			} else {
				this.RemoveBetCart(selectionData, 2, true);
			}
		} else if (overMaxSelectionCount) {
			Toasts.error(' Số lượng cược tối đa có thể đóng' + Vendor.configs.MaxParlay);
		}
	};
	...
	checkMaintenanceStatus(name) {
		...
		console.log(isIM,isLogin,'noTokenIMnoTokenIMnoTokenIM',noTokenIM)
		switch (name) {
			case 'bti':
	...
	imMaintenanceView(className) {
		return <View style={[styles.noDataView1, {backgroundColor: isBlue? '#efeff4': '#212121',}]}>
			<Image source={require("../../../images/euroSoprt/euroCup/tool.png")} style={{ height: 62, width: 62 }} />
			<Text style={{ color: isBlue? '#666666': '#fff', lineHeight: 30 }}>Hệ thống đang nâng cấp</Text>
			<Text style={{ color: isBlue? '#999999': '#fff', }}>Hiện tại thể thao IM đang được bảo trì. </Text>
			<Text style={{ color: isBlue? '#999999': '#fff', }}> Vui lòng quay lại sau.</Text>
		</View>
	}

	//無數據
	noDataView(className) {
		return <View style={[className ? styles.noDataView1 : styles.noDataView,{backgroundColor: !className ? isBlue? '#fff': '#212121': isBlue? '#EFEFF4': '#000',}]}>
			<Image source={require("../../../images/euroSoprt/euroCup/nodata.png")} style={{ height: 62, width: 62 }} />
			<Text style={{ color: isBlue? '#666666': '#fff', lineHeight: 30 }}>Không Có Dữ Liệu</Text>
			<Text style={{ color: isBlue? '#999999': '#fff', fontSize: 12, }}>Không có trận đấu bóng đá giải Euro hôm nay,</Text>
			<Text style={{ color: isBlue? '#999999': '#fff', fontSize: 12, }}> vui lòng chọn ngày khác.</Text>
		</View>
	}

	headerDataChange() {
		setTimeout(() => {
			window.HeaderData && window.HeaderData(this.state.EventDatas)
		}, 1000);
	}
	...
	render() {
		...
		if (EventDatas !== null && EventDatas.length > 0) {
			...
			groupByDateEventDatas = dateList.map(dd => {
				...
				return {
					dateString: this.weekDayMap[dd.isoWeekday()] + ' ' + dd.format('DD/M '),
					dateString1: dd.format('DD/MM ') + ' ' + this.weekDayMap[dd.isoWeekday()],
							...
							: (
								(EventDatas !== null && EventDatas.length <= 0)
									...
									<View style={styles.EmptyBox}>
										<Image source={require("../../../images/euroSoprt/euroCup/nodata.png")} style={{ height: 62, width: 62 }} />
										<Text style={{ color: '#666666', lineHeight: 30 }}>Không Có Dữ Liệu</Text>
										<Text style={{ color: '#999', fontSize: 12, textAlign: 'center' }}>{EC2021IsEnd ? 'Giải đấu EURO đã kết thúc.\nHãy sẵn sàng cho các sự kiện sắp tới.' : 'Các trận đấu EURO hiện tại chưa mở cược.\nVui lòng quay lại sau.'}</Text>
									</View>
									: //展示數據
									(DisplayType === 1)
										? //直列表
										(isIM_Maintenacne
											...
											: groupByDateEventDatas.map(dateInfo => {
												return <View key={dateInfo.dateString1} style={{ backgroundColor:  isBlue?'#fff': '#212121' }}>
													<View style={{ width: width, backgroundColor: isBlue? '#EFEFF4': '#000', paddingLeft: 20 }}>
														<Text style={{ color: isBlue?'#000': '#959595', fontWeight: 'bold', lineHeight: 44 }}>{dateInfo.dateString1}</Text>
													</View>
													{
										...
										<View>
											<ScrollView
												horizontal={true}
												style={[styles.dateTabs,{borderBottomColor: isBlue? '#E0E0E0': '#000',}]}
											...
											<View style={{ backgroundColor: isBlue?'#fff': '#212121' }}>
												{
													isIM_Maintenacne
